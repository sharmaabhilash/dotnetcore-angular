...
namespace DatingApp.API.Controllers
{
    ...
    public class MessagesController : ControllerBase
    {
        ...
        [HttpGet("{recipientId}", Name = "GetMessage")]
        public async Task<IActionResult> GetMessage(int userId, int recipientId)
        {
            if(userId != int.Parse(User.FindFirst(ClaimTypes.NameIdentifier).Value))
                return Unauthorized();
            
            var messageFromRepo = await _datingRepository.GetMessage(recipientId);
        ...
        public async Task<IActionResult> CreateMessage(int userId, MessageForCreationDTO messageForCreationDTO)
        {
            ...
            if(await _datingRepository.SaveAll())
            {
                ...
                return CreatedAtRoute("GetMessage", new { recipientId = message.Id }, messageToReturn);
            }
